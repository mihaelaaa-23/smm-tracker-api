...
export default function DataTransferModal({ onClose }: Props) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const ok = await confirm({
      ...
      message: 'This will replace all existing clients, tasks, and payments with the data from the backup file. This cannot be undone.',
    ...
    try {
      ...
      setStatus({ type: 'success', message: `Imported ${imported} records successfully.` })
    } catch (err: any) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl w-full max-w-sm p-8 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">
              Restores data from a backup file. All existing data will be overwritten.
            </p>
            <button
      ...
      {dialogProps.open && <ConfirmDialog {...dialogProps} />}
    </>
  )
}